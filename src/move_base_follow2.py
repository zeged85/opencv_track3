...
class LineFollower(object):
	def __init__(self):
		...
		self.scan = rospy.Subscriber("/tb3_0/scan", LaserScan, self.scan_callback, queue_size=1)
		self.laser_values = np.full(360, np.inf)
		self.regions = {}
		self.target_found = False
		self.object_location = None
		self.pub_ = rospy.Publisher('/tb3_0/cmd_vel', Twist, queue_size=1)
	...
	def follow(self):
		...
		if self.target_found:
			...
			if mid<1.5:
				...
				vel = min(abs(mid - keep_distance),0.3)
			else:
				#drive(1,ang)
				vel = 0.3
